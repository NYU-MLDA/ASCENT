...
class ReplayBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        super().__init__(buffer_size, action_space, device, n_envs=n_envs)
        ...
        if not optimize_memory_usage:
            ...
            self.next_observations = [None for _ in range(self.buffer_size)]
    ...
    def add(
        ...
        if self.optimize_memory_usage:
            self.observations[(self.pos + 1) % self.buffer_size] = copy.deepcopy(next_obs)#np.array(next_obs)
        else:
            self.next_observations[self.pos] = copy.deepcopy(next_obs) #np.array(next_obs)
    ...
    def _get_samples(self, batch_inds: np.ndarray, env = None) -> ReplayBufferSamples:
        ...
        env_indices = 0 # Only one environment is there
        obs = [self.observations[idx] for idx in batch_inds]
        if self.optimize_memory_usage:
            #next_obs = self._normalize_obs(self.observations[(batch_inds + 1) % self.buffer_size, env_indices, :], env)
            next_obs = [self.observations[(idx+1)%self.buffer_size] for idx in batch_inds]
        else:
            next_obs = [self.next_observations[idx] for idx in batch_inds]
...
class RolloutBuffer(BaseBuffer):
        ...
        if not self.generator_ready:
            ...
            for tensor in _tensor_names:
                if tensor == "observations":
                    continue # Observation is of type dictionary,
    ...
    def _get_samples(
        ...
        #     [self.observations[idx] for idx in batch_inds],
...
#     observations: List #Dict[str, np.ndarray]  # type: ignore[assignment]
#     next_observations: List #Dict[str, np.ndarray]  # type: ignore[assignment]
...
#         handle_timeout_termination: bool = True,
#     ):
#         super(ReplayBuffer, self).__init__(buffer_size, action_space, device, n_envs=n_envs)
...
#             total_memory_usage: float = self.actions.nbytes + self.rewards.nbytes + self.dones.nbytes
#             if not optimize_memory_usage:
#                 next_obs_nbytes = 0
#                 # for _, obs in self.observations.items():
