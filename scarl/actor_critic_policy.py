...
class BaseModel(nn.Module):
    ...
    def dict_to_pyg(self, observation):
        """
...
class BasePolicy(BaseModel, ABC):
    ...
    def predict(
        ...
        if not isinstance(dict_observation, Dict):
            raise ValueError(
...
class ActorCriticPolicy(BasePolicy):
    ...
    def forward(self, dict_observation: dict, deterministic: bool = True) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        features = self.extract_features(dict_observation)
        if self.share_features_extractor:
    ...
    def extract_features(  # type: ignore[override]
        ...
        """
        if self.share_features_extractor:
            return super().extract_features(dict_observation, self.features_extractor)
        else:
            pi_features = super().extract_features(dict_observation, self.pi_features_extractor)
            vf_features = super().extract_features(dict_observation, self.vf_features_extractor)
    ...
    def _predict(self, pyg_observation: Data, deterministic: bool = True) -> th.Tensor:
        ...
        :return: Taken action according to the policy
        """
        return self.get_distribution(pyg_observation).get_actions(deterministic=deterministic)

    def evaluate_actions(self, dict_observation: dict, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        """
    ...
    def get_distribution(self, dict_observation: dict) -> Distribution:
        """
    ...
    def predict_values(self, dict_observation: dict) -> th.Tensor:
        """
